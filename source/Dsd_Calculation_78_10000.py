...
def calc_nd(sr_event, dict_speed, dsize=.125, area=45.6 / (100 ** 2), dt=60):
    ...
    for speed in sr_event.index:
        if pd.notnull(sr_event.loc[speed]):
            ...
            ND += sr_event.loc[speed] / (dspeed * area * dt * dsize)
    return ND
...
def calc_nt(sr_event, dict_speed, dsize=.125, area=45.6 / (100 ** 2), dt=60):
    ...
    for speed in sr_event.index:
        if pd.notnull(sr_event.loc[speed]):
            dspeed = dict_speed[speed]
            NT += (sr_event.loc[speed] / (dspeed * area * dt))
    return NT
...
def calc_W(sr_event, dict_speed, dsize=.125, area=45.6 / (100 ** 2), dt=60):
    ...
    for speed in sr_event.index:
        ...
        if pd.notnull(sr_event.loc[speed]):
            dspeed = dict_speed[speed]
            W += (sr_event.loc[speed] / (dspeed * area * dt)) * dsize ** 3
    return W
...
def calc_Z(sr_event, dict_speed, dsize=.125, area=45.6 / (100 ** 2), dt=60):
    ...
    for speed in sr_event.index:
        ...
        if pd.notnull(sr_event.loc[speed]):
            dspeed = dict_speed[speed]
            Z += (sr_event.loc[speed] / (dspeed * area * dt)) * dsize ** 6
...
dsd_file = 'dsd_data_78_nh_sorted_2016.csv'
...
list_speed = {0: .2, .2: .2, .4: .2, .6: .2, .8: .2,
              1.: .4, 1.4: .4, 1.8: .4, 2.2: .4, 2.6: .4, 3.: .4,
              3.4: .8, 4.2: .8, 5.0: .8, 5.8: .8, 6.6: .8, 7.4: .8, 8.2: .8,
              9.: 1.,
              10.: 10.}
...
for chunk in chunks:
    ...
    dsd_results['rain_mred'] = dsd_data_final[9]
    ...
    for event_ext in idx_events:
        ...
        try:
            ...
            dsd_results.loc[event_ext, 'Z_mp'] = Z_mp
            ...
            dsd_results.loc[event_ext, 'NT_exp'] = N_0exp
...
np.savetxt(
    '{0}/{1}/CG_relation_z1_{2}_{3}.xlsx'.format(path_results, dsd_number, dsd_number, num_gotas), z, delimiter=",")
